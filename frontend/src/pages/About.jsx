...
const About = () => (
  <>
    ...
    <div className="flex flex-col items-center min-h-screen bg-[#18181b] text-white px-4 py-10">
      <h1 className="text-4xl font-bold mb-4 text-blue-400 text-center">About CodeSphere</h1>
      <p className="max-w-2xl text-lg text-center mb-8">
        <span className="font-semibold text-blue-400">CodeSphere</span> is an online code editor and project management platform.<br />
        You can create, edit, and run code in multiple languages, manage your projects, and get instant AI-powered code assistance.
      </p>
      <div className="w-full flex flex-wrap justify-center gap-6 mb-10">
        {benefits.map((b, i) => (
          <div
            key={b.title}
            className="bg-[#23232a] rounded-xl shadow-lg p-6 w-72 flex flex-col items-center hover:scale-105 transition-transform"
          >
            <div className="text-4xl mb-3">{b.icon}</div>
            <h2 className="text-xl font-semibold mb-2 text-blue-300 text-center">{b.title}</h2>
            <p className="text-gray-300 text-center">{b.desc}</p>
          </div>
        ))}
      </div>
      <div className="max-w-xl text-gray-300 text-center mb-8">
        <h2 className="text-2xl font-semibold text-blue-300 mb-2">How CodeSphere Helps You</h2>
        <ul className="list-disc list-inside text-left mx-auto max-w-md mb-4">
          <li>Accessible anywhere—just open your browser, no installation needed.</li>
          <li>Instant feedback and output for faster learning and debugging.</li>
          <li>AI assistant helps you understand, fix, and improve your code.</li>
          <li>Organize your work and revisit projects anytime.</li>
          <li>Perfect for students, professionals, and teams alike.</li>
        </ul>
        <p>
          Made with <span className="text-red-400">❤</span> using React, Node.js, and modern web technologies.
        </p>
      </div>
    </div>
  </>
